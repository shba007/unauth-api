export default defineProtectedEventHandler<AuthResponse>(async (event, user) => {
  ...
  if (!(user && user.phone)) throw createError({ statusCode: 400, statusMessage: 'Send OTP First' })
  ...
  try {
    if (phoneStatus.otp !== otp)
      throw createError({
        statusCode: 401,
        statusMessage: "OTP don't match",
      })

    if (new Date(phoneStatus.otpTimeout).getTime() < Date.now()) throw createError({ statusCode: 400, statusMessage: 'OTP expired' })
